...
import (
	...
	"github.com/bernardjkim/ptrade-api/cmd/server/pkg/types/users"
...
// GetDayHistory returns today's portfolio history
func (h *PortfolioHandler) GetDayHistory(w http.ResponseWriter, r *http.Request) {
	...
	history, err := ORM.GetDayHistory(h.DB, userID)
	if err != nil {
		log.Println(err)
		http.Error(w, "Unable to get portfolio history", http.StatusInternalServerError)
		return
